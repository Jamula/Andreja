...
| Help escalation from a feedback case | The controller for the originating intake remains controller for the escalation and decides whether escalation is necessary. | A contracted help/escalation vendor is a processor only for the documented escalation purpose and approved fields; counsel, authorities, or affected people receive information only under the incident/legal conditions below. |
...
| --- | --- | --- | --- | --- |
| `schemaVersion`, `feedbackId`, `sourceChannel`, `feedbackType`, `submittedAt`, `affectedSurface`, optional `affectedVersion`, `correlationId`, `status`, optional `statusReasonCode`, `ownerRef`, `retentionClass` | Accept, route, secure, and explain a requested feedback case | LI; Contract only for a necessary support request. Counsel must confirm by jurisdiction | Restricted intake and assigned support roles; content-free correlation data may reach approved operational processors | Review after 90 days without activity; close after 180 days without activity unless a documented continuing need is communicated. No active record exceeds 365 days from submission unless one owner-approved renewed-need extension of at most 90 days is recorded with a 30-day review and expiry date. After closure retain the minimized record for 180 days, then hard-delete |
...
| Proof token and anonymous rights-session record: purpose-bound token hash, request reference, scope, issued/used/expiry times, proof level/result, attempt counter, and revocation state | Let an accountless or non-user requester securely retrieve a private outcome or appeal without using a display reference as a credential | LI or Contract for the private rights route | Restricted rights service and privacy caseworkers; never ordinary intake staff, public systems, or GitHub | Token expires after 30 days or one successful retrieval, whichever comes first; revoke on outcome, deletion, or abuse threshold; purge the token hash and session record 30 days after expiry or final appeal |
...
| Diagnostic consent receipt (`consentReceipts` entry): receipt ID, subject/source reference, purpose, exact field scope or digest, notice version, affirmative action, time, withdrawal state/time | Prove what was shown and authorized; enforce withdrawal | Legal accountability/LI hypothesis; the underlying diagnostic action uses Consent | Privacy/compliance roles; the diagnostic service receives only a current authorization result | Three years after withdrawal or private-record purge, whichever is later; retain no diagnostic value in the receipt |
| Notice version, locale, rendered fields, operator/contact configuration, and publication/retention summary | Prove which notice governed a submission and keep disclosures aligned with policy | Legal accountability/LI hypothesis; consent is valid only against the displayed version | Privacy/compliance roles and the notice renderer; no provider or public recipient | Retain with the related consent or case receipt for three years after withdrawal, publication, or private-record purge, then hard-delete |
| Optional `repositoryIssueRef` and sanitized draft digest/version | Prevent duplicate publication, bind consent to an exact draft, and operate public status | LI for mapping; Consent for disclosure | Restricted publisher and repository maintainers. Submitter sees only a safe public link where appropriate | Mapping and digest remain while the public artifact exists and for 180 days after its removal; re-review every 12 months while retained and delete at the next review after five years unless a separately documented hold applies; no tracking secret or private ID in GitHub |
| Saved pre-publication draft: exact title/body/labels, digest/version, preview time, repository/visibility, and consent state | Render and bind the exact candidate before publication without treating an unconsented draft as public | LI for controlled preparation; Consent is required before disclosure | Restricted publisher and privacy/compliance roles; no GitHub or ordinary support access | Delete when consent is refused or withdrawn, or 30 days after preview if no publication occurs; retain only a minimized audit reference when a scoped hold or completed publication requires it |
| Publication receipt: receipt ID, source reference, repository/visibility, exact draft digest/version, notice version, consent time, publisher, publication URL/object ID, withdrawal/removal state and time | Prove and reconcile an authorized public disclosure without retaining private content | Consent for publication; legal accountability/LI hypothesis for the receipt | Restricted privacy/compliance and publisher roles; repository maintainers receive only the public result | Retain three years after withdrawal or controlled artifact removal, then hard-delete; independent public copies remain outside Andreja's retention control |
| Sanitized GitHub issue/comment created from intake | Maintain a public project work item after separate publication consent | Consent for the initial disclosure; LI for necessary project maintenance, subject to objection and balancing | Public or repository collaborators, GitHub as independent controller, and uncontrolled downstream readers/copies | Review at closure and 24 months later, then every 12 months. Remove personal content immediately when discovered; after 24 months minimize stale content to what remains necessary for project history or remove it, and delete the controlled artifact at the next review after five years unless a separately documented hold applies. Independent copies follow their controllers' rules |
...
| Lifecycle/activity history and ownership-transfer audit evidence: event, actor/role, owner transition, reason, timestamp, and case reference | Explain routing, assignment, reopening, and ownership changes without copying case content | LI for accountability and security | Restricted support, privacy, security, and audit roles; no public or ordinary analytics access | Retain content-free events for three years after private-record purge, then hard-delete; a scoped hold may preserve exact events only |
| Deletion job: job ID, source scope, requested/completed times, retry state, destination set, and tombstone reference | Execute and prove idempotent deletion across active and derived stores | Legal accountability/LI hypothesis; the covered deletion uses the source record's basis | Restricted privacy, security, and operations roles; processors receive only scoped deletion instructions | Retain three years after completion or final failure resolution, then hard-delete; never retain deleted content in the job |
| Destination receipt: destination, scoped record class, job ID, result, time, and provider/object reference | Reconcile propagation and support a safe completion claim | Legal accountability/LI hypothesis | Restricted privacy and operations roles; no case body or public disclosure | Retain three years after completion or final failure resolution, then hard-delete |
| Restricted exception-queue entry: destination, failure class, retry count, scope, owner, review/expiry, and safe resolution | Block false completion claims and manage unresolved propagation failures | LI for security and rights protection | Restricted privacy/security/operations roles; no ordinary support or public access | Delete on verified resolution plus 90 days; unresolved entries require a 30-day review and expire no later than one year unless a scoped hold applies |
| Backup deletion tombstone: source scope, record classes, digest, creation time, expiry, and replay status | Prevent deleted content from returning after backup restore | Legal accountability/LI hypothesis; same covered record basis | Restricted backup operators and privacy/security roles; no content payload | Retain until all backups containing the scope expire, then 90 additional days for replay evidence, then hard-delete |
...
> Attachments are not accepted. The service may also collect short-lived
> network/rate signals, challenge results, abuse counters, delivery events, and
> content-free activity/access records for security, delivery, rights handling,
> and audit purposes; their retention is described in the schedule below.
>
...
> current provider and subprocessor list, locations, and transfer safeguards are
> at [provider notice link]. A contracted help-escalation provider may receive
> only the approved fields needed for that escalation. Counsel, authorities, or
> affected people may receive limited incident information only when the
> approved incident/legal conditions require it.
>
...
1. Acknowledge privately with a random request reference and no existence
   confirmation. Offer a separate purpose-bound, one-time proof token or
   anonymous rights session; the display reference is never a credential.
...
messages, metrics generation, and deletion of the disputed fields themselves
while a decision is pending, or of fields covered by an approved hold.
...
and documented continuing need. One owner-approved renewed-need extension may
continue active handling for no more than 90 days beyond the 365-day submission
ceiling, with a 30-day review and recorded expiry. Further extension requires a
new decision package; otherwise the case closes and its retention clock starts.
A new notice where required or a scoped hold may apply separately.
