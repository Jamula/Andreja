...
Checksums detect corruption but do not authenticate an exporter. A signed OCI
artifact is trusted only when verified against a separately trusted operator anchor.
...
| OCI trust and update/rollback | Supply-chain scripts/policy and #44 matrix | **Evidence-blocked** |
