...
- **Execution window:** original run 2026-08-24 15:41–17:14 PDT; cumulative rerun 2026-09-07 11:54–11:55 PDT
...
| Formatting, dependency advisories, and documentation contracts | Data | `dotnet format Andreja.slnx --verify-no-changes --no-restore`;<br>`dotnet format tests/Andreja.PostgreSqlIntegrationTests/Andreja.PostgreSqlIntegrationTests.csproj --verify-no-changes --no-restore`;<br>`pwsh -NoProfile -File .github/scripts/invoke-nuget-vulnerability-scan.ps1`;<br>`python3 -m unittest tests/test_check_docs_consistency.py`;<br>`python3 .github/scripts/check_docs_consistency.test.py`;<br>`python3 .github/scripts/check_docs_consistency.py`;<br>`python3 scripts/docs/generate_architecture_diagram.py --check`;<br>`python3 scripts/docs/test_architecture_diagram.py` | **PASS:** both format checks, the direct/transitive advisory scan, 19 docs-consistency unit tests, 20 plan/hash/catalog tests, nine status-artifact hashes, deterministic architecture rendering, and 21 PNG structural tests passed. |
| Hosted .NET validation at the cumulative source revision | Data / Tuvok | [run 34153528787](https://github.com/Jamula/Andreja/actions/runs/34153528787) at `0b8edac57e050cb5d51e0b4597b690cbd76b26c3`; [Debug job](https://github.com/Jamula/Andreja/actions/runs/34153528787/job/101840511742), artifact `dotnet-Debug-evidence-1` (`sha256:d8e5aff2615b3ada0ad693090735c6870885bcdd51218ce6b6d7bc1613018b80`); [Release job](https://github.com/Jamula/Andreja/actions/runs/34153528787/job/101840511478), artifact `dotnet-Release-evidence-1` (`sha256:ccb18299c071c7d33c18523321a48c77411b1672342c9f2556a3214d12b2ac6f`); [format job](https://github.com/Jamula/Andreja/actions/runs/34153528787/job/101840511677), artifact `dotnet-format-evidence-1` (`sha256:9a073bde667820af89fa18dab4cb9836b8ff9a83d6d12f4125b17e6b875cf463`); [NuGet audit job](https://github.com/Jamula/Andreja/actions/runs/34153528787/job/101840511667), artifact `nuget-vulnerability-evidence-1` (`sha256:37fc4d1470aabf574ff72c5eeaddde650bdb5bfc63c6c223b3b1ae37f0e4765a`) | **PASS:** Debug and Release solution build/test, separate Debug and Release PostgreSQL-project compiles, formatting, and NuGet vulnerability audit all succeeded. Artifacts share the 2026-09-17 18:54:53 UTC retention deadline; the hosted jobs did not execute PostgreSQL runtime tests. |
| Hosted C# SAST (DevSkim) at the cumulative source revision | Tuvok / Data | [.NET Validation run 34153528787, job 101840511727](https://github.com/Jamula/Andreja/actions/runs/34153528787/job/101840511727); [csharp-sast-evidence-1](https://github.com/Jamula/Andreja/actions/runs/34153528787/artifacts/10030174103), artifact ZIP `sha256:116597819704f0ecc861f255e7c87e0ec947881d18e733a61c6eb884f2c61f62` | **PASS WITH RECORDED WARNINGS:** DevSkim `1.0.90` analysis and finding gate passed with zero findings at the exact cumulative commit. Scope was `**/*.cs`, excluding `**/.git/**,**/bin/**,**/obj/**,**/artifacts/**,**/.tools/**`; baseline rule exclusions were `DS137138` (loopback URLs) and `DS162092` (Debug-guard false positives). This is DevSkim evidence, not C# CodeQL or runtime coverage. Node/action deprecation warnings were emitted; requested 14-day artifact retention was capped at 10 days, expiring 2026-09-17 18:54:53 UTC. |
...
| Evidence harness static validity | Data | `node --check scripts\evidence\browser-e2e.mjs`;<br>`pwsh -NoProfile -Command '& { $tokens = $null; $errors = $null; [System.Management.Automation.Language.Parser]::ParseFile("scripts/evidence/Test-OfflineEvidence.ps1", [ref]$tokens, [ref]$errors) > $null; if ($errors.Count) { $errors \| Format-List; exit 1 } }'`;<br>`pwsh -NoProfile -Command '& { $tokens = $null; $errors = $null; [System.Management.Automation.Language.Parser]::ParseFile("scripts/evidence/Test-TelemetryEvidence.ps1", [ref]$tokens, [ref]$errors) > $null; if ($errors.Count) { $errors \| Format-List; exit 1 } }'` | **PASS (syntax only):** all three scripts parsed. No browser, Compose, offline, or telemetry runtime result is inferred. |
| Hosted OCI build/audit at the cumulative source revision | Jett Reno / Data | [OCI Supply Chain run 34153528772](https://github.com/Jamula/Andreja/actions/runs/34153528772) | **PASS WITH RECORDED WARNINGS (unsigned audit only):** the ordinary `push` run for the exact cumulative commit produced manifest `sha256:573c255b7e30fa85bb696568a73d7757fafe043d6e879f5c67723166e7ea0a64` and config `sha256:e039a1eb8e310d4ccd106c16828d7063a8cefcabb2fcb57e84ddf6a4159588b1`. Restore emitted repeated NU1801 source-index warnings, Trivy fell back to embedded checks because cached policy content was absent, and runner/action Node deprecation warnings were emitted. The run passed, but it was not a version-tag event, so the keyless signing job did not run and cannot authorize release, startup, or update. |
| Keyless version-tag signing and offline verification | Tuvok / Jett Reno | `.github\workflows\oci-supply-chain.yml`;<br>`pwsh -NoProfile -File scripts\supply-chain\Test-OciEvidence.ps1 -BundleDirectory artifacts\supply-chain -TrustedPolicyPath D:\trusted\supply-chain-policy.json -TrustedRootPath D:\trusted\sigstore-trusted-root.json -ExpectedSigningMode keyless-sigstore` | **BLOCKED:** ADR 0010's implementation and 36 policy vectors are present, but repository history contains no qualifying `v*` push run or retained signed bundle for this revision. A reviewed tag decision and independently authenticated root are external prerequisites. |
...
| Compose browser, passkey, viewport, reconnect, provider-failure, offline, and all-signal OTel evidence | Data / Jett Reno | `node scripts\evidence\browser-e2e.mjs`;<br>`pwsh -NoProfile -File scripts\evidence\Test-OfflineEvidence.ps1 -AuditedAppImage $env:ANDREJA_IMAGE`;<br>`pwsh -NoProfile -File scripts\evidence\Test-TelemetryEvidence.ps1 -CollectorMetricsUrl http://127.0.0.1:18888/metrics -PrometheusUrl http://127.0.0.1:19090 -MinimumRequestSpanDelta 10` | **UNAVAILABLE, NOT RUN:** run these commands from the repository root in PowerShell with the evidence stack configured. `ANDREJA_IMAGE` must be the audited application reference (`repository@sha256:` plus 64 lowercase hex digits) for the revision under test, matching Compose; all Compose images must be immutable and preloaded for the offline gate. Browser/telemetry require Edge (`EDGE_PATH` if non-default), the local TLS certificate and bootstrap-token file, the reachable app, and Collector/Prometheus on the specified ports. These prerequisites remain unavailable. The prior run below remains bounded to its historical digest. Human assistive-technology review also remains unavailable. |
...
OCI audit and C# DevSkim finding gate. It explicitly excludes PostgreSQL runtime execution, all
