# Phase 1A threat, privacy, cost, and test gates

- **Status:** Proposed
- **Issue:** [#9](https://github.com/cyrusjamula/Andreja/issues/9)

These are internal dogfood gates, not public SLAs or production claims.
...
| Contract/conformance | API DTO serialization, assistant fake/failure/cancel, skill/channel manifest and permission negatives |
...
| Operations | Offline startup, restart, update, provider outage, dump/restore plus keys, app import, readiness |
...
| Content-policy telemetry violations | exactly 0 | `sum(increase(andreja_telemetry_policy_violation_total[7d]))` |
...
Zero denominators produce "insufficient evidence," never success. Every evidence
record includes build digest, schema version, window, sample count, query version,
owner, and known exclusions. RPO/RTO and retention windows remain human decisions.
