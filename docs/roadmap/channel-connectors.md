...
| 3 | Assistant providers — deterministic fake, OpenAI-compatible BYOK; real GitHub Copilot OAuth only after explicit #74 decision; later Azure AI/local | Query/generate (assistant provider calls) | Seven of Nine (provider abstraction); Jett Reno (channel packaging) | MVP | Provisional pending #74: fake/BYOK in 1A; Copilot in gated 1B | Identity/Tenancy/Authorization | Charter pending | [plan §Assistant and AI architecture](../plan.md#assistant-and-ai-architecture) | #74 remains the open decision of record. Under the conservative provisional scope, Phase 1A has no real Copilot provider. GitHub assistant auth, content connector, and feedback publishing use three distinct tokens/grants, never reused. |
