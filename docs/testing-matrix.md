...
| Passkey user handle | A generated ES256 assertion exercises the .NET 10 `PasskeyHandler` with the reserved bootstrap Identity ID; exact `userHandle` resolves and a different handle fails. Debug cookie sign-out proves the endpoint clears authentication | `dotnet test Andreja.slnx --no-build` | Automated; real browser ceremony remains separately blocked |
